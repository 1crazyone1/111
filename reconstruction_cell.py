...
from cell import generator as generator_gray
from cell import checkpoint as checkpoint_gray
from cell import dataset
# from cell import val_set
...
#prepare CS
np.random.seed(2)
randn_B = np.random.randn(CS_ratio,image_size*image_size)
...
# dir_root_test = pathlib.Path('D:/data/cell_data/test_data')#16张测试图片
# dir_path_test = list(dir_root_test.glob('*.jpg'))
# dir_path_test.sort()
#
# test_num = 0
# x_test = np.zeros([16,image_size,image_size,1])
...
# val_set = val_set.map(preprocess).batch(16)
# print(len(val_set))
# print(val_set)

dir_root_test = pathlib.Path('D:/data/cell_data/test')#256张
dir_path_test = list(dir_root_test.glob('*.jpg'))
dir_path_test.sort()

test_num = 0
x_test = np.zeros([256,image_size,image_size,1])
for img_path in dir_path_test:
    img_path = os.path.join(img_path)
    img = load_lung(img_path)
    x_test[test_num] = img
...
val_set = tf.data.Dataset.from_tensor_slices((x_test))
val_set = val_set.map(preprocess).batch(256)
print(len(val_set))
print(val_set)
...
def Generator():
    ...
    # initializer = tf.random_normal_initializer(0., 0.02)
    initializer = tf.keras.initializers.VarianceScaling(scale=0.02, mode='fan_in', distribution='uniform', seed=None)
    dense1 = tf.keras.layers.Dense(new_image_size * new_image_size, activation='relu')#1024
    reshape1 = tf.keras.layers.Reshape((CS_ratio,)) # length
    reshape2 = tf.keras.layers.Reshape((new_image_size, new_image_size, 1)) #(32,32,1)
    ...
    x = last(x)
    # new_last = tf.keras.layers.Conv2D(OUTPUT_CHANNELS, 10, strides=1, activation='relu')  # FL用relu yao gai =>28,28
    new_last = tf.keras.layers.Conv2D(OUTPUT_CHANNELS, 10, strides=1,activation=tf.keras.layers.Activation('tanh')) #FL用tanh
    x = new_last(x)
...
checkpoint_dir = './training_checkpoints_Reconstruction_cell4_1'
checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt")
checkpoint = tf.train.Checkpoint(generator_optimizer=generator_optimizer,
                                 generator=generator_unet)

log_dir="logs_Reconstruction_cell4_1/"
summary_writer = tf.summary.create_file_writer(
  log_dir + "fit_Reconstruction_cell4_1/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def compute_signal(images,image_gan):
    ...
    for i in range(BATCH_SIZE):
        real_image = np.array(images[i]).reshape(image_size * image_size, 1)
        fake_image = np.array(image_gan[i]).reshape(image_size * image_size, 1)
    ...
    # print(min1,max1)
    # print(min2,max2)
    true_signal = true_signal / 200
    false_signal = false_signal / 200
...
def start_train(images,epoch):
    ...
    with tf.GradientTape() as gen_tape:
        image_gan = generator_gray(noise, training=True)
        true_signal,false_signal = compute_signal(images,image_gan)
        # print(false_signal.shape)
...
def cal_psnr_ssim(pre_images_gan,pre_images_unets):
    ...
    img1 = np.array(img1)
    img2 = np.array(img2)
    img1 = tf.image.convert_image_dtype(img1, tf.float64)
    img2 = tf.image.convert_image_dtype(img2, tf.float64)
    psnr = tf.image.psnr(img1, img2, max_val=1.0)
    ssim = tf.image.ssim(img1, img2, max_val=1.0)
    return psnr,ssim

def generate_image(generator_gray,test_noise,generator_unet,epoch):
    pre_images_gan = generator_gray(test_noise, training=False)
    ...
    for i in range(BATCH_SIZE):
        ...
        plt.axis('off')
        plt.savefig("./reconstruction_cell4_1/"+str(epoch)+"_" +"_unets"+ ".png")
    #plt.show()
...
def generate_image_test(image_signal,generator_unet,images,epoch):
    ...
    #savemat(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\matrix\matrix1\img1_t_%d.mat" %(epoch), {"Array_img1":img1})
    #savemat(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\matrix\matrix2\img2_t_%d.mat "%(epoch), {"Array_img2":img2})

    # new_path_1 = "D:/data/cell_data/cv2_cell/" + str(epoch) + "_1" + ".jpg"
    # new_path_2 = "D:/data/cell_data/cv2_cell/" + str(epoch) + "_0" + ".jpg"
    ...
    for i in range(2):
        ...
        plt.axis('off')
        if i == 1:
            plt.savefig("./reconstruction_cell_test4/" + str(epoch) + "_" + ".png")
...
def fit(dataset,epochs):
    ...
    for epoch in range(epochs):
        ...
        generate_image(generator_gray, seed, generator_unet,epoch)
    checkpoint.save(file_prefix=checkpoint_prefix)
...
def test_data(image_signal, generator_unet, images, epoch):
    for i in range(images.shape[0]):
        ...
        # new_path_1 = "D:/data/cell_data/cv2_cell/cell_378_two/" + str(epoch) + "_1" + ".jpg"
        # new_path_2 = "D:/data/cell_data/cv2_cell/cell_378_two/" + str(epoch) + "_0" + ".jpg"

        new_path_1 = "D:/data/cell_data/cell_new_test/ours/cell_189/" + str(epoch) + "_1" + ".jpg"
        new_path_2 = "D:/data/cell_data/cell_new_test/ours/cell_189/" + str(epoch) + "_0" + ".jpg"
        # cv2.imwrite(new_path_1, img1*255.0)
        # cv2.imwrite(new_path_2, img2*255.0)
        save_img(new_path_1, img1)
        save_img(new_path_2, img2)
        ...
        #     plt.axis('off')
        #     if i == 1:
        #         plt.savefig("./reconstruction_cell_test4_2/" + str(epoch) + "_" + ".png")
        epoch = epoch + 1


# checkpoint.restore(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\training_checkpoints_Reconstruction_cell2\ckpt-2")
# checkpoint.restore(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\training_checkpoints_Reconstruction_cell3\ckpt-2") #cs_ratio = 378
# checkpoint.restore(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\training_checkpoints_Reconstruction_cell3_1\ckpt-2") #cs_ratio = 378
# checkpoint.restore(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\training_checkpoints_Reconstruction_cell4\ckpt-2") #cs_ratio = 189
checkpoint.restore(r"C:\Users\程龙\PycharmProjects\pythonProject\image_cell\training_checkpoints_Reconstruction_cell4_1\ckpt-2") #cs_ratio = 189
# fit(dataset,EPOCH)
num = 0
total_num = 16
# dataset
# for images in dataset.take(total_num):
#     images_signal,pre = compute_signal(images,images)
#     # print(images.shape, images_signal.shape)
...
def compute_val_signal(images):
    ...
    for i in range(test_num):
        ...
        true_signal[i, :, :] = real_signal  # 真实信号
    true_signal = true_signal / 200
    # max = tf.reduce_max(true_signal)
    # min = tf.reduce_min(true_signal)
    # print(max,min)
    true_signal = true_signal.astype(np.float32)
    return true_signal
